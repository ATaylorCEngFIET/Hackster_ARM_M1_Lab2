...
# Adjust NUM_IRQ widths to allowed values
proc adjust_width { width } {
  if {$width <= 1} {
    return 1
  } elseif {$width <= 8} {
    return 8
...
proc post_propagate { cellpath otherInfo } {
  ...
  if { [string length $intrDriver] > 0 } {
    ...
    # - Adjust width to allowed values: 1, 8, 16 or 32
    set width [get_property CONFIG.PortWidth $intrPin]
    set max_width 32
    if { [string length $width] > 0 && $width != 0 } {
      if {$width <= $max_width} {
        set_property CONFIG.NUM_IRQ [adjust_width $width] $cell
      }
      ...
      if {$userLevelEdge} {
        ...
        if {($nLevelEdge & $mask) != ($curLevelEdge & $mask)} {
          bd::send_msg -of $cellpath -type WARNING -msg_id 7 -text \
            ": Interrupt level/edge manual value ($curLevelEdge) does not match computed value ($strLevelEdge)."
        }
      } else {
        bd::send_msg -of $cellpath -type INFO -msg_id 9 -text \
          ": Setting Interrupt level/edge value to $strLevelEdge."
      ...
      if {$userHighLow} {
        ...
        if {($nHighLow & $mask) != ($curHighLow & $mask)} {
          bd::send_msg -of $cellpath -type WARNING -msg_id 8 -text \
            ": interrupt high/low manual value ($curHighLow) does not match computed value ($strHighLow)."
        }
      } else {
        bd::send_msg -of $cellpath -type INFO -msg_id 10 -text \
          ": Setting Interrupt high/low value to $strHighLow."
